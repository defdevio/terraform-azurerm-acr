...
func TestCreateGeoreplicatedAzureContainerRegistry(t *testing.T) {
	tfVars := terraformVars()
	tfVars["georeplications"] = map[string]any{
		"dr": map[string]any{
			"location": "eastus",
		},
	}
	...
	// Defer the deletion of the resource group until all tests have finished
	defer helpers.DeleteAzureResourceGroup(t, subscriptionID, resourceGroup)

	// Defer destroying the terraform resources until the rest of the tests finish
